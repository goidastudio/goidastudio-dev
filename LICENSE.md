# Лицензионное соглашение для GhostOS, Ghost browser и остальных наших продуктов. ↓↓↓




# Лицензионное соглашение GOS

© 2025 Goida Studio. Все права защищены.

## 1. Определения
- **GOS** — операционная система, включая все её компоненты, документацию и обновления.
- **GPS (GOS Processes Security)** — изолированная среда для запуска приложений без доступа к реальному оборудованию.
- **GSSM (GOS Security Starting Machine) ** — механизм анализа приложений на уровне ОС перед их выполнением.

## 2. Условия использования
1. **Название "GOS"** является торговой маркой (™). Запрещено:
   - Использовать название в своих продуктах без письменного разрешения.
   - Регистрировать домены/аккаунты со словом "GhostOS".
   - Использовать созвучные названия и другие вариации этого названия. 
2. **Функционал ОС** (GPD, GSSM):
   - Запрещено декомпилировать, модифицировать или обходить защитные механизмы.
   - Любое коммерческое использование требует согласования.
3. **Распространение**:
   - Разрешено только оригинальное ПО с этого репозитория.
   - Запрещено включать GhostOS в сторонние дистрибутивы без лицензии.

## 3. Авторские права и лицензия
- Все права сохранены. Код закрыт.
- Для использования в коммерческих продуктах — свяжитесь с нами.

## 4. Ответственность
- GOS предоставляется "как есть". Мы не несем ответственности за:
  - Ущерб от использования ОС.
  - Ложные срабатывания системы проверки безопасности.

## 5. Нарушения
При несоблюдении условий:
1. Вы теряете право использования GoidaOS.
2. Мы оставляем за собой право подать жалобу:
   - На GitHub (DMCA).
   - В суд со штрафом до 5 млн ₽, если вы в РФ, и до 15 млн ₽, если вы не в РФ. Штраф может быть больше если мы понесли ущерб болле 1₽.

---


# Лицензионное соглашение Ghost Browser

© 2025 Goida Studio. Все права защищены.

## 1. Определения
- **Ghost Browser™** (включая вариации: GHOST BROWSER, GhostBrowser и иные производные) — веб-браузер и все его компоненты.
- **Уникальные функции**:
  - Анти-фишинг (проверка ссылок до открытия).
  - Блокировка переадресаций.
  - Запрет внешних ссылок (требование перетаскивания).
  - Анти-майнинг (детекция и блокировка скриптов майнинга).

## 2. Защита названия
- Запрещено:
  - Использовать названия **Ghost Browser**, **GHOST BROWSER**, **GhostBrowser** и любые производные в других продуктах.
  - Регистрировать домены/аккаунты, содержащие эти названия.
  - Упоминать в метаданных (HTML, репозиториях) без разрешения.

## 3. Защита функционала
Любая реализация следующих функций без лицензии запрещена:
1. **Анти-фишинг**: предварительный анализ ссылок перед открытием.
2. **Проверка безопасности URL**: сканирование перед загрузкой страницы.
3. **Блокировка переадресаций**: запрет HTTP 301/302 без согласия пользователя.
4. **Механизм внешних ссылок**:
   - Требование перетаскивания для открытия внешних ресурсов.
   - Блокировка прямых переходов.
5. **Анти-майнинг**:
   - Детекция скриптов Cryptojacking.
   - Обязательное подтверждение пользователя для выполнения.

## 4. Условия использования
- **Для пользователей**:
  - Разрешено только официальное использование браузера.
  - Запрещен реверс-инжиниринг и модификация кода.
- **Для разработчиков**:
  - Любое копирование функций требует письменного согласия.
  - Использование в коммерческих продуктах — только по лицензии.

## 5. Лицензия на код
- Все права сохранены. Код закрыт.
- Для использования в коммерческих продуктах — свяжитесь с нами.

## 6. Ответственность
- Ghost Browser предоставляется "как есть".
- Мы не несем ответственности за:
  - Ложные срабатывания защиты.
  - Ущерб из-за блокировки "легитимных" скриптов.

## 7. Нарушения
При несоблюдении условий:
1. Нарушитель теряет право использования.
2. Мы оставляем за собой право:
   - Подать DMCA-жалобу (GitHub, хостинги).
   - Обратиться в суд со штрафом до 5 млн ₽, если вы в РФ, и до 15 млн ₽, если вы не в РФ. Штраф может быть больше если мы понесли ущерб болле 1₽.


---


# Лицензионное соглашение Anti AI Clean

© 2025 Goida Studio. Все права защищены.

## 1. Определения
- **Anti AI Clean™** (включая вариации: ANTI AI CLEAN, AntiAIClean, Anti-AI-Clean и иные производные) - программное обеспечение и все его компоненты.
- **Уникальные функции**:
  - Полная очистка текста от форматирования
  - Удаление следов генерации ИИ (скрытые пробелы, невидимые символы, мета-теги)
  - Восстановление оригинальной структуры текста
  - Детекция AI-генерации

## 2. Защита названия
Запрещено:
- Использовать названия **Anti AI Clean**, **ANTI AI CLEAN**, **AntiAIClean** и любые производные
- Создавать созвучные названия (например, "AntiAI Clear", "AI Cleaner Pro")
- Регистрировать домены/аккаунты с содержанием этих названий

## 3. Защита функционала
Любая реализация следующих возможностей без лицензии запрещена:
1. **Очистка от форматирования**:
   - Удаление скрытых символов
   - Нормализация пробелов и табуляции
2. **Удаление следов ИИ**:
   - Выявление и очистка мета-тегов генерации
   - Удаление "воды" и шаблонных фраз

## 4. Условия использования
- **Для пользователей**:
  - Разрешено только официальное использование ПО
  - Запрещен реверс-инжиниринг и модификация
- **Для разработчиков**:
  - Любое копирование алгоритмов требует согласия
  - Коммерческое использование - только по лицензии

## 5. Лицензия на код
- Все права сохранены. Код закрыт.
- Для использования в коммерческих продуктах — свяжитесь с нами.

## 6. Ответственность
- Anti AI Clean предоставляется "как есть"
- Мы не несем ответственности за:
  - Ложные срабатывания детекции
  - Потерю важного форматирования

## 7. Нарушения
При несоблюдении условий:
1. Нарушитель теряет право использования
2. Мы оставляем за собой право:
   - Подать DMCA-жалобу
   - Обратиться в суд со штрафом до 5 млн ₽, если вы в РФ, и до 15 млн ₽, если вы не в РФ. Штраф может быть больше если мы понесли ущерб болле 1₽.

---


**Контакты для запросов:**  
https://goidastudio.github.io/goidastudio-dev/